...
- 打刻開始時の `currentTime` は離し操作まで保留し、同じ再生タイムラインのまま完了した場合だけ編集データへ反映する。
- `play`、`pause`、`seeking`、`ratechange`、音源切替、タブ離脱で再生タイムラインが変化した場合、進行中の打刻は破棄する。`seeking` 中の新規打刻も拒否する。
- Undo で打ち直し位置へ移動する場合は、`seeked` を待ってから再生する。
...
- Synthesizer Vとの連携
- ブラウザUIからのSVP、MIDI、Synthesizer V APIの読み込み（既存JSONを非破壊で復旧する運用スクリプトは除く）
- 音声のサーバーアップロード
- JSONへの音声バイナリ埋め込み
